...
public class TicTacToeActivity4x4 extends Activity {

	// Da Game Handler
	private TicTacToe4x4 game;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// get the players names passed from previous activity prompt
		if (!gameType) {
			if (!getIntent().getExtras().getString("_PLAYER_1_NAME").isEmpty())
				_PLAYER_1_NAME = getIntent().getExtras().getString(
						"_PLAYER_1_NAME");

			if (!getIntent().getExtras().getString("_PLAYER_2_NAME").isEmpty())
				_PLAYER_2_NAME = getIntent().getExtras().getString(
						"_PLAYER_2_NAME");
		}
		...
		boardButtons[9] = (Button) findViewById(R.id.ten);
		boardButtons[10] = (Button) findViewById(R.id.eleven);
		boardButtons[11] = (Button) findViewById(R.id.twelve);
		boardButtons[12] = (Button) findViewById(R.id.thirteen);
		boardButtons[13] = (Button) findViewById(R.id.fourteen);
		boardButtons[14] = (Button) findViewById(R.id.fifteen);
		boardButtons[15] = (Button) findViewById(R.id.sixteen);

		infoTextView = (TextView) findViewById(R.id.information);

		// create a new 4x4 TicTacToe game object
		game = new TicTacToe4x4();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.newGame:
			...
			break;
		case R.id.exitGame:
			TicTacToeActivity4x4.this.finish();
			break;
		}
		return true;
	}
	...
	// implements an onClickListener for button locations
